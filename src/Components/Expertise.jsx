...
// Expanded expertise data with categories, descriptions and icons
const expertiseData = [
  { 
...
const Expertise = () => {
  return (
    <section className="py-16 px-6 md:px-20 text-white">
      ...
      >
        <h2 className="text-5xl font-bold text-violet-500 text-center mb-4">My Expertise</h2>
        ...
        {expertiseData.map((category, categoryIndex) => (
          <div key={categoryIndex} className="mb-12">
            <motion.h3 
              className="text-2xl font-semibold text-violet-400 mb-6 border-b border-violet-800 pb-2"
            ...
            >
              {category.skills.map((item, index) => (
                <motion.div 
                  key={index} 
                  className="mb-8 bg-black bg-opacity-30 backdrop-blur-sm p-6 rounded-lg shadow-lg hover:shadow-violet-900/20 transition-all duration-300 border border-gray-800"
                  variants={itemVariants}
                >
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-violet-400 text-xl">{item.icon}</span>
                    <span className="text-xl font-semibold text-gray-200">{item.skill}</span>
                    <span className="text-lg font-semibold text-violet-400 ml-auto">{item.percentage}%</span>
                  </div>
        ...
        <motion.div 
          className="mt-12 p-6 bg-black bg-opacity-30 backdrop-blur-sm rounded-lg border border-violet-900"
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          transition={{ delay: 0.5, duration: 0.7 }}
          viewport={{ once: true }}
        >
          <h3 className="text-xl font-semibold text-violet-400 mb-3">Currently Learning</h3>
          <div className="flex flex-wrap gap-3">
            {["AWS Cloud, App Development, ReactNative, Advanced Data Structues"].map((item, index) => (
              <span 
                key={index} 
                className="px-3 py-1 bg-violet-900 bg-opacity-40 rounded-full text-violet-200 text-sm"
              >
                {item}
              </span>
            ))}
